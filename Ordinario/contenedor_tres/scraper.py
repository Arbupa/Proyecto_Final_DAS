	...
	def animeforgenreid(self):
		...
		for i in range(1, 43):
			...
			while j < 5:
				...
				page_id = data['results'][j].get('mal_id')
				...
				datos = {"page_id": page_id, "title": title, "image": image, "episodes": episodes, "airing": airing, "tipo": tipo, "start_date": start_date, "end_date": end_date, "members": members, "rated": rated}
				print(datos)
				print(i)
				lista.append(datos)
				j += 1
	...
	def mangaforname(self):
		...
		for i in name:
			...
			while cont < 5:
				...
				datos = {"page_id": page_id, "image_url": image, "title": title, "publishing": publishing, "synopsis": synopsis, "tipo": tipo, "chapters": chapters, "volumes": volumes, "start_date": start_date, "end_date": end_date}
				lista.append(datos)
				cont += 1
		print(lista)
		return lista
...
# resultados = Scraper()
# resultados.animedetaillsid()
# time.sleep(6)
# resultados.animedetaillsname()
# time.sleep(16)
